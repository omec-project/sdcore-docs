...
The easiest way to configure SD-Core is to use Simapp. Simapp is a Kubernetes pod that
reads YAML configuration and automatically configures subscribers, device groups, and
network slices in SD-Core.

Simapp Initial Configuration Helm Values
-----------------------------------------

The following YAML configuration shows the initial setup for Simapp:
...
To add subscribers to an already running 5G core:

1. Edit the Simapp configuration:

   .. code-block:: bash

      kubectl edit configmap simapp -n <namespace>

2. Update the ``subscribers`` section in the configuration
3. Save and exit the editor

The changes will be automatically applied within 60 seconds. In the example below, we update
``ueId-end`` from ``208014567891205`` to ``208014567891210``, which adds 5 new subscribers.
...
To remove subscribers from an already running 5G core:
...
2. Update the ``subscribers`` section to reduce the range
3. Save and exit the editor

The changes will be automatically applied within 60 seconds. In the example below, we update
``ueId-end`` from ``208014567891210`` to ``208014567891208``, which removes 2 subscribers
from the 5G core.
